...
parser.add_argument('--content_name', type=str, default="boat",
                    help='Image resolution')
parser.add_argument('--exp_name', type=str, default="multicrop_0.3",
                    help='Image resolution')
...
for epoch in range(0, steps+1):
    ...
    for n in range(num_crops):
        src_patch, trg_patch = paired_crop(content_image, target, crop_size = args.crop_size)
        trg_patches.append(augment(trg_patch).to(device))
        src_patches.append(augment(src_patch).to(device))

    trg_patches = torch.cat(trg_patches,dim=0)
    src_patches = torch.cat(src_patches,dim=0)

    trg_features = clip_model.encode_image(clip_normalize(trg_patches,device))
    trg_features /= (trg_features.clone().norm(dim=-1, keepdim=True))

    src_features = clip_model.encode_image(clip_normalize(src_patches,device))
    src_features /= (src_features.clone().norm(dim=-1, keepdim=True))

    img_direction = torch.nn.functional.normalize(trg_features-src_features,dim=-1)
    ...
    text_direction = (text_features-text_source).repeat(src_features.size(0),1)
